...
| **NEW: pin the site's own build in CI** | `tests/test_site_pages.py` runs the builder and asserts the committed pages are byte-identical to a fresh build | The site is a build artifact rendered from `data/evidence/*.json`; without this a page can silently go stale against the evidence it claims to render | 4 tests, all passing |
