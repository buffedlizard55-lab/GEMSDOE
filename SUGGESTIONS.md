...
2. **Read the re-fired fold-0 union contrast** when it lands (`data/evidence/block_holdout/fold0_heldout_combined.json` + the pseudo arm's `combined` population) — the pre-registered bars are **P ≥ 0.95 and +0.010 on the union, held-out blocks** before a pseudo field may even be proposed to the field-selection gate.
