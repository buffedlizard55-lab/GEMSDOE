...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><b>MEASURED 2026-09-19 — a third truth population now exists.</b> <code>scripts/block_holdout_eval.py --combined-population</code> scores the disjoint union of the catalogue labels and the new-fault-like proxy pixels the labels do not contain (<b>122,652 px</b> = 60,988 + 61,664, disjointness verified at score time), the closest local surrogate for the expanded Phase-2 truth of rules §3.6. The shipped artifact scores <b>0.2074, CI95 [0.1929, 0.2229]</b> there (components: catalogue 0.2298, proxy-only 0.0999) and the adopted emission policy <b>holds</b>: the only better candidate in the sweep is width 1 px at <b>+0.0054 with P = 0.815</b>, below <i>both</i> pre-registered bars (P ≥ 0.95, +0.010) — <code>data/evidence/proxy/combined_truth_shipped.json</code>. <b>Training-side selection is now built and verified in-proof (2026-09-21).</b> <code>src/train.py</code> logs the shaped DTI of every epoch on BOTH the in-domain labels and the union population (<code>DTI_union=</code>, the exact summator for which is unit-tested on synthetic grids), so the union signal this row asked for is now produced on every fold without a GPU; wiring it into early stopping / ensemble weights is the remaining (heavier) step, queued below.</td>
      <td>Select on the <b>union</b>, never on its components: <code>FP_w</code> sums over <i>prediction</i> pixels, so the union's DTI cannot be inferred from the two component DTIs (<code>TP_w</code>/<code>FN_w</code> do add — the populations are disjoint). Wire <code>training.select_on</code> (union vs in-domain) into the early-stopping comparator; the per-epoch union DTI logging this row's build adds is the feed-in, and every fold field committed from now on carries that population (<code>--combined-population</code> is in both training workflows).</td>
    </tr>
    <tr>
      <td>5</td>
