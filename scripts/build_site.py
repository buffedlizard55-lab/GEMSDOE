...
<p>The pipeline now runs end-to-end on the real competition rasters, but placing top-5 on the leaderboard (current best 0.1972) requires moving from <b>measured plumbing fixes</b> to <b>geophysical detection gains</b>. The table below prioritizes the remaining work by impact and risk, and marks what is already queued for the next session(s). Each row traces to an evidence file or to the resource-limit analysis in <a href="results.html">Results</a> and <code>SUGGESTIONS.md</code>.</p>
...
<div class="note warn"><b>How to work on these in the next session(s).</b> Items <b>1</b> and <b>5</b> are <i>closed by measurement</i>: the cross-catalogue transfer refused itself because the training labels and QFaults are the same lines in this footprint (1 code-2 pixel of 60,939), and more folds measurably did not help the field (0.0850 vs 0.0999). Item <b>2</b> has its fold-0 gap (+0.0114) committed and folds 1–3 in flight; item <b>7</b> is built and in flight (fold 0 with SGMC pseudo labels, paired against the no-pseudo baseline). Which <b>field</b> ships is no longer a judgment call — the pre-registered rule in §6 (F1/F2 + R1–R5, R3 required, default KEEP) is machine-enforced on every re-blend, and the committed verdict is KEEP mean12. The remaining upside is <b>8</b> (human: upload and read the public leaderboard — the only source of unbiased signal) → <b>3</b> (1 m DEM derivatives) → <b>6</b> (GPU capacity) → <b>4</b> (selection on a new-fault-like population), then read the landed <b>2</b>/<b>7</b> evidence and decide the next experiment from it. Items 3–6 require an unrestricted machine and/or GPU; the bridge already supplies the competition rasters.</div>
""")
