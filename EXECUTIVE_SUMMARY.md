...
> **Suggested order for next session:** items **1** and **2** are now *closed by measurement* (1 refused on a total label/QFaults overlap; 2 reproduced bit-for-bit on a runner), and **5** is deferred because more folds measurably did not help. The remaining upside is therefore **8 (human: upload and read the public leaderboard) → 3 (1 m DEM derivatives) → 6 (GPU capacity) → 4 (selection on a new-fault-like population) → 7 (pseudo-labels under spatial holdout)**. Items 3–7 require an unrestricted machine/GPU; the bridge already supplies the competition rasters. Item 8 is only human-gated — until upload, every number here remains a local proxy and the public leaderboard is the first unbiased signal.
