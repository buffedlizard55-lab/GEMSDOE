...
|---|---|---|---|---|
| 1 | **Fix detection, not just width — cross-catalogue transfer** | 74% of new-fault-like truth >12 px (1.2 km) from any emitted pixel; widening to 16 px lifts DTI only 0.0247→0.0713. Remaining error is detection, not localization (`miss_distance-ensemble1.json`). | **Built 2026-09-18, measurement dispatched to a runner** (the service is unreachable from the sandbox). `scripts/fetch_qfaults.py` pages USGS QFaults layer 21 for the footprint with an integrity gate (`fetched == service-reported`; 14,482 features verified live) and a provenance sidecar; `scripts/measure_cross_catalogue_transfer.py` runs the controls first, emits catalogue A at widths 0–6 px, scores against QFaults code-2 pixels, and tests `union(model, A)` against a **pre-registered** criterion (gain > 0.01 DTI **and** P(union > model) ≥ 0.95 over block resamples). `.github/workflows/cross-catalogue.yml` fires both. | Read `data/evidence/xcat/transfer_report.json` when the run lands: `verdict.conclusion` is `ADOPT` / `DO NOT ADOPT` / `REFUSED` / `NOT MEASURABLE`. Only on `ADOPT` does an external catalogue join the emission — then train with it as auxiliary supervision. |
| 2 | **Spatial block-holdout (solve in-domain/proxy sign conflict)** | In-domain DTI (0.1903→0.0908 when widening) and proxy DTI (+0.11) disagree on sign — choosing one corrupts other. | **Measured 2026-09-18.** `scripts/block_holdout_eval.py` scores the shipped submission per 51.2 km block and bootstraps over blocks: proxy DTI **0.0999, CI95 [0.0883, 0.1119]**; the best genuinely different alternative (width 1 px, 0.0878) beats the reference with probability **0.008**, so the adopted policy is confirmed *with an error bar* rather than by point estimate. Training-side holdout is wired too (`configs/config_block_holdout.yaml`, `--score-fold K [--complement]`), and the scoring partition is cross-checked against the training partition at runtime. | Fire `.github/workflows/block-holdout.yml` with `TRAIN_FOLDS=0,1,2,3` for the four-fold generalisation gap; until then the per-block numbers are a **reshaping** measurement on a model that saw every block (the report says so in `restriction.note`). |
